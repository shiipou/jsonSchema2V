...
fn test_load_from_string() {
	...
	type_ := schema.type_
	assert typeof(type_).idx == typeof[string]().idx, 'assertion failed for typecheck type: $type_'
	assert type_ == 'component', 'assertion failed for type: $type_'


	prop_type := schema.properties['type'] or { panic('property type not found') } as JSONSchemaProperty 
